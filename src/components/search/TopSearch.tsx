...
const buttonStyle = (disabled: boolean) => {
  ...
  return {
    ...
    backgroundColor: !disabled ? "white" : "#E7F5FB",
  };
};
...
const TopSearch = ({ page }: any) => {
  ...
  const searchBtnHandler = (disabled: boolean) => {
    if (!disabled) {
          ...
          {suggestions.map((item: any) => {
            return (
              <Button
                key={item.text}
                text={item.text}
                btnStyles={buttonStyle(item.disable)}
                btnHandler={(e: any) => searchBtnHandler(item.disable)}
